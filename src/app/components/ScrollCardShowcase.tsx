...
export default function ScrollCardShowcase() {
  ...
  useEffect(() => {
    ...
    const scrollTween = gsap.to(trackRef.current, {
      x: (i) => -centerOffset(cards.length - 1),
      ease: 'none',
      paused: true,
    })
    ...
    >
      <div className="sticky top-0 h-screen flex flex-col justify-center items-center px-6">
        ...
        <div ref={infoRef} className="text-center mb-8">
          <h1 className="font-russo text-4xl mb-2">
        ...
        <div className="relative w-full max-w-[100vw] overflow-hidden">
          ...
          >
            {cards.map((card, i) => (
              <div
                key={card.name}
                className={`w-[260px] shrink-0 transition-transform duration-500 ${
              ...
              >
                <div className="">
                  <img
                    src={card.image}
                    alt={card.name}
                    className=" h-[240px] object-contain mb-4"
                  />
                </div>
              </div>
            ))}
